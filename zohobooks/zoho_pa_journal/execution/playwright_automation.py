...
class ZohoJournalAutomation:
    ...
    async def run(self, run_date: str):
        async with async_playwright() as p:
            self.logger.info(f"Starting Journal Automation for {self.settings['org_name']} (Date: {run_date})")
            ...
            try:
                ...
                self.logger.info("Clicking 'Export Journals' from sub-menu...")
                await page.evaluate('''() => {
                    const items = Array.from(document.querySelectorAll('button, a, li, span'));
                    const target = items.find(el => el.textContent.trim() === 'Export Journals' && !!(el.offsetWidth || el.offsetHeight));
                    if (target) target.click();
                }''')
                ...
                self.logger.info("Ensuring 'Journal Date' filter is selected...")
                await page.evaluate('''() => {
                    const labels = Array.from(document.querySelectorAll('label'));
                    const target = labels.find(l => l.textContent.trim() === 'Journal Date');
                    if (target) target.click();
                }''')
                ...
                try:
                    ...
                    save_path = self.download_dir / f"journal_{run_date.replace('-','')}.xlsx"
                ...
                await page.screenshot(path="error_journal.png")
                return None
            finally:
                await context.close()
