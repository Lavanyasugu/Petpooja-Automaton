...
PROJECTS = {
    "orders": "/home/admin/petpooja/pporder",
    "item-orders": "/home/admin/petpooja/ppitemorder",
    "growth-report": "/home/admin/petpooja/ppgrowthreport"
}
...
def send_summary_email(summary_data: list):
    ...
    subject = f"Petpooja Automation Summary - {date_str}"
    
    body = f"<h2>Daily Automation Report - {date_str}</h2>"
    ...
    body += "</table>"
    body += "<p>Detailed logs are available on the server at /home/admin/petpooja/webhook/execution.log</p>"
...
def run_script(project_path: str) -> bool:
    ...
    try:
        ...
        with open("/home/admin/petpooja/webhook/execution.log", "a") as f:
...
async def trigger_project(
    ...
    if project_id == "all":
        ...
        return {"status": "success", "message": "All projects queued for sequential execution"}
    
    if project_id not in PROJECTS:
        raise HTTPException(status_code=404, detail="Project not found")
    
    background_tasks.add_task(run_script, PROJECTS[project_id])
    return {"status": "success", "message": f"Project {project_id} queued for execution"}
